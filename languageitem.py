"""
LanguageItem is the smallest language item to learn.
It could be words, phrases or even whole sentences.
But the main idea is not to learn languageitem separatelly
but via sentences (statements, questions, answers) that is usage examples.
...
WhatIf: create separate LanguageItem and UsageCases (tree structure that can grow,
each node should contain appropriate difficulties level)
...
class TxtAudioResource(str):
    ...
    def play_list(cls, txt_audio_resource_list, delay, *largs):
        """
        Play list of elements with desired time interval.
        Elements should be played one by one in turn and depending on previous run.
        Clock.schedule_once will run in recursion.
        """
...
Class public interface
    question - provide randomly selected question from all accululated questions that
    were got passing classes hierarchy. Answer for all these question should be only one!
    answer - provide first statement in the first super class (assumption)
    sentence - provide randomly selected statement from all accululated statements
    that were got passing classes hierarchy

"""
    @classmethod
    def accumulate(cls, self, get_values_function_name, accumulated_field):
        """ Walk via hierarchy of classes started from cls.
...
Previous version:
There is a issue what we statically bind 'function' to upper class
for which accumulate is invoked!!!
    pass function name - not a way as inheritance will not work -
    we need to pass classified name like cls.function"""
        visited_classes = set()
        def go_through_classes(cls, self):
            ...
            if cls.__bases__:
                #print ('bases')
                #old version - function in statically binded to highest class function.
...
class _LexisFrame(str):
    """Contains pattern to create language expression.

    Longer infor here

    Example:
        'this is {noun}.', 'what is this?', 'Ask what it is.'

    Attributes:
    """

class _StatementLexisFrame(_LexisFrame):
    """
    """
    pass

class _QuestionLexisFrame(_LexisFrame):
    """
    """
    pass

class _AnswerLexisFrame(_LexisFrame):
    """
    """
    pass

class _InstructionFrame(_LexisFrame):
    """
    """
    pass

class UsageCase(dict):
    """Contains dict of _LexisFrame (Statement, Question, Answer, Instruction)
    to learn to make statement, question, answering. Methods to operate them.

    UsageCase contains:
        static: Format string for Statement, Question, Answer, Instruction expressions
        dymanic: Values of LanguageItem with proper form to populate Format string
    

    UsageCase contains format string or already string populated with values???
        If class contains format string than it is necessary to create lot of classes because
        each UsageCase = new Class - so we will have lots of classes
        If class contains nothing - then we need to populate with format strings and values
    So class can be string format but object of this class should be populated string.
    Also it is possible to create fabric class

    Format string + list of values for each frame
        Format string is static for class/object
        Values are dynamic and depends on LanguageItem (grammar form)
    So LanguageItem is combination of Grammar and UsageCases patterns that produce UsageCases 

    Example: 
        Statement (compulsory) - listen and repeat (single. but two times)
            possitive (this is a cat)
            negative (this is not a cat)
        Question - listen and answer (two frame)
        Instruction - listen and question (two frame)

        Animal = UsageCase ( , bases=[class1, class2])

    Attributes:
    """
    _usage_case = {}
    _usage_case['statement'] = None
    _usage_case['question'] = None
    _usage_case['answer'] = None
    _usage_case['instruction'] = None

    def __init__ (self, statement_format='', question_format='', answer_format='', instruction_format=''):
        self._statement_format = statement_format
        self._question_format = question_format
...
class LanguageItem(TxtAudioResource, Statements):
    """
Look like LanguageItem is static object what is not changing in time - state is the same!!!
So we do direct access to data.
...
"""
    fields = {'states': [], 'meaning': None, 'part': None}
    ...
    def __init__(self, text, audio_source = None, **extra):
        pass
        #self.register() # add language_item to lexises

    def register(self):
        """add language_item to lexises"""
        lexis.lexises.add(self)

    @staticmethod
    def create(class_name, *argv, **kwargv):
        """return LanguageItem object of cls class = fabric function"""
        return eval('%s' % class_name)(*argv, **kwargv)
        #return speechparts.__dict__[class_name](*argv, **kwargv)
...
class LanguageItemForm(TxtAudioResource):
...
"""
    fields = {'level': None, 'countable': True, 'uncountable': False, 'pluralform': None, 'plural': False, 'article': None, 'live': False, 'grammar_person': 3}
    ...
    def _statement_values(cls, self): #if patterns class related - then values should be class related too but add dynamic - should be redefined 
        return [(this_pronoun.form(plural=self.plural),
                 ToBe.form(plural=self.plural, person=3, time='presence'),
                 self.usage_form())]
    @classmethod
    ...
    def usage_form(self):
        """Add article is necessary and return str"""
        if self.uncountable or self.plural:
            return self
        else:
            return '%s %s' % (self.get_article(), self)
    def form(self, plural=False, article=False):
        """There are singular and plural forms for noun"""
        if article:
            return LanguageItemForm(self.usage_form(), self, article=True)
        elif plural:
            return LanguageItemForm(self.get_pluralform(), self, plural=plural)
        else:
            return self
...
class Adjective(Word):
    """ Describe Adjective lexis and its case usage"""
    @classmethod
    def _question_values(cls, self = None):
        return []
...
#mixing classes
class Herbivore(NounUsageForm):
    """an animal that eats only plants"""
    _statement_patterns = [['%s eats plants.', '%s eats grass, and, leaves.']]
class Carnivore(NounUsageForm):
    """an animal that eats meat"""
    _statement_patterns = [['%s eats animals.', '%s eats meat.']]
class Land(NounUsageForm):
    """an animal what can live on land and has 4 legs, can walk and run"""
    _statement_patterns = [['%s has four legs', '%s can walk.', '%s can run.']]
class Water(NounUsageForm):
    """an animal what can live in water and can swim"""
    _statement_patterns = [['%s can swim.']]
class HasTail(NounUsageForm):
    """ mixing class to describe has tail property """
    _statement_patterns = [['%s has a tail.']]
class FurSkin(NounUsageForm):
    """ mixing class to describe fur skin """
...
class Verb(Word):
    ...
    Verb form can depend on:
        time
        subject (from, plural/singular, gender)
...
class ToBe(Verb):
    ...
    """
    @staticmethod
    def form(person=3, time='presence', plural=False, subject=None):
        if time == 'presence':
            if subject:
...
class PossessiveDeterminer(Determiner):
    ...
    @staticmethod
    def form(sex='male', grammar_person=1, plural=False, noun=None):
        """
        expect person as noun
        """
        if noun:
           sex = noun.sex
           plural = noun.plural
        ...
        if plural:
            if grammar_person == 1:
                ...
                raise AttributeError(grammar_person)
        else:
            if grammar_person == 1:
                return 'my'
            elif grammar_person == 2:
...
class Pronoun(Word):
    ...
    def form(self, plural=False, noun=None):
        if noun:
            if noun.plural:
                return LanguageItemForm(self.pluralform, self, plural=noun.plural)
            else:
                return self
        else:
            if plural:
                return LanguageItemForm(self.pluralform, self, plural=plural)
            else:
                return self
...
def load_nouns(dict_yaml):
    """
    """
    dict_yaml = {'animals': [['a', 'cat', 'A1'], ['a', 'dog', 'A1'], ['a', 'cow', 'A1']],
                 'vehicles': [['a', 'car', 'A1'], ['a', 'bus', 'A1']],
                 'clothes': [['a', 'dress', 'A1'], ['a', 't-shirt', 'A1']]}
...
"""
    ...
    grammar_person = [1, 2, 3]
    plural = [True, False]
    for i_sex in sex:
        for i_person in grammar_person:
            for i_plural in plural:
                print(i_sex, i_person, i_plural, PossessiveDeterminer.form(sex=i_sex, plural=i_plural, grammar_person=i_person))
